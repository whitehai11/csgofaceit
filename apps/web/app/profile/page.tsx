...
export default async function ProfilePage() {
  ...
  if (!session) {
    return <div className="card p-6">Profile requires login.</div>;
  }
  ...
  if (!steamId) {
    return <div className="card p-6">Could not resolve your Steam account.</div>;
  }

  const profileRes = await fetch(`${API_BASE_URL}/users/${encodeURIComponent(steamId)}/profile`, { cache: "no-store" }).catch(() => null);
  ...
  const isOwnProfile = steamId === String(player.steam_id ?? "");
    ...
    <div className="space-y-6">
      <section className="card p-6">
        ...
        <div className="mt-4 flex items-center gap-4">
          {player.avatar_url ? (
            ...
            <div className="flex h-16 w-16 items-center justify-center rounded-full border border-white/20 bg-white/5 text-xl font-bold">
              {initial}
            </div>
          )}
          <div>
      ...
      <section className="card p-6">
        ...
        <div className="mt-4 space-y-2">
          {history.map((item) => (
            <div key={item.match_id} className="flex items-center justify-between rounded-xl border border-white/10 bg-white/5 p-3">
              <div className="flex items-center gap-3">
                <span className={`rounded px-2 py-1 text-xs font-semibold ${item.result === "Win" ? "bg-emerald-500/20 text-emerald-300" : "bg-red-500/20 text-red-300"}`}>
                  {item.result}
                </span>
                <span className="text-sm text-white/85">{item.map}</span>
              </div>
              <span className="text-sm text-white/70">{item.score}</span>
            </div>
          ))}
          {history.length === 0 && <p className="text-sm text-white/70">No recent matches.</p>}
        </div>
      </section>
    </div>
  );
